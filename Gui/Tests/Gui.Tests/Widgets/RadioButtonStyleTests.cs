...
[TestFixture]
public class RadioButtonStyleTests
{
    ...
    [Test]
    public void Default_MapsPrimaryToDotColor()
    {
        var primary = new Vector4(0.8f, 0.1f, 0.3f, 1);
        var style = RadioButtonStyle.Default(Scheme(primary), TextTheme.Default());
        Assert.That(style.DotColor, Is.EqualTo(primary));
    }
    ...
    public void Default_MapsSurfaceHighToBackgroundColor()
    {
        ...
        var style = RadioButtonStyle.Default(Scheme(surfaceHigh: surfaceHigh), TextTheme.Default());
    ...
    public void Default_MapsBorderToBorderColor()
    {
        ...
        var style = RadioButtonStyle.Default(Scheme(border: border), TextTheme.Default());
    ...
    public void Default_MapsOnSurfaceToLabelStyleColor()
    {
        ...
        var style = RadioButtonStyle.Default(Scheme(onSurface: onSurface), TextTheme.Default());
    ...
    public void Default_MapsBodyFontSizeToLabelStyleFontSize()
    {
        ...
        var style = RadioButtonStyle.Default(Scheme(), text);
        Assert.That(style.LabelStyle.FontSize, Is.EqualTo(text.Body.FontSize));
    }


    [Test]
    public void WithNoStyle_UsesThemeDotColor()
    {
        var dotColor = new Vector4(0.1f, 0.9f, 0.3f, 1);
        var themeStyle = RadioButtonStyle.Default(ColorScheme.Default(), TextTheme.Default()) with
        {
            DotColor = dotColor
        };
        var theme = new ThemeData(radioButtonStyle: themeStyle);
        var (_, root) = MountTree(new Theme(theme, new RadioButton<int>(1, 1)));

        Assert.That(FindRenderBoxWithColor(root, dotColor), Is.Not.Null);
    }

    [Test]
    public void WithStyleOverride_UsesOverrideDotColor()
    {
        var overrideColor = new Vector4(0.9f, 0.1f, 0.5f, 1);
        var style = RadioButtonStyle.Default(ColorScheme.Default(), TextTheme.Default()) with
        {
            DotColor = overrideColor
        };
        var (_, root) = MountTree(new RadioButton<int>(1, 1, style: style));

        Assert.That(FindRenderBoxWithColor(root, overrideColor), Is.Not.Null);
    }

    [Test]
    public void WithStyleOverride_IgnoresThemeDotColor()
    {
        ...
        var theme = new ThemeData(radioButtonStyle: themeStyle);
        var overrideStyle =
            RadioButtonStyle.Default(ColorScheme.Default(), TextTheme.Default()) with
            {
                DotColor = overrideColor
            };
        var (_, root) =
            MountTree(new Theme(theme, new RadioButton<int>(1, 1, style: overrideStyle)));
