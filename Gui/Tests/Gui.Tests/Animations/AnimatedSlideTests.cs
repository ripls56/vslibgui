...
public class AnimatedSlideTests
{
    ...
    [Test]
    public void SlideChange_AnimatesOverTime()
    {
        var (element, owner, vsync) = Mount(
            Vector2.Zero, TimeSpan.FromMilliseconds(200));

        element.Update(new AnimatedSlide(
            new Vector2(1f, 0.5f),
    ...
    [Test]
    public void InitialOffset_Zero_ProducesIdentityTransform()
    {
        ...
        var ro = FindRenderTransform(element);
        Assert.That(ro, Is.Not.Null);
        Assert.That(ro!.Matrix.TransX, Is.EqualTo(0f).Within(0.01f));
        Assert.That(ro!.Matrix.TransY, Is.EqualTo(0f).Within(0.01f));
    }
